defmodule SimpleAuth.Accounts.AccountToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  schema "accounts_tokens" do
    ...
    belongs_to :account, SimpleAuth.Accounts.Account
  ...
  def build_session_token(account) do
    ...
    {token, %SimpleAuth.Accounts.AccountToken{token: token, context: "session", account_id: account.id}}
  end
  ...
  defp build_hashed_token(account, context, sent_to) do
    ...
    {Base.url_encode64(token, padding: false),
     %SimpleAuth.Accounts.AccountToken{
       token: hashed_token,
       context: context,
       sent_to: sent_to,
  ...
  def token_and_context_query(token, context) do
    from SimpleAuth.Accounts.AccountToken, where: [token: ^token, context: ^context]
  end
  ...
  def account_and_contexts_query(account, :all) do
    from t in SimpleAuth.Accounts.AccountToken, where: t.account_id == ^account.id
  end

  def account_and_contexts_query(account, [_ | _] = contexts) do
    from t in SimpleAuth.Accounts.AccountToken, where: t.account_id == ^account.id and t.context in ^contexts
  end
end
